...
<div class="container eagle-container">
  ...
  <div class="jumbotron eagle-jumbotron-2">
    <div class="row eagle-row" style="margin: 0; padding: 0 1rem 0 1rem">
      <h1 class="col-10 eagle-h2" style="display: flex; align-items: center; padding: 0">Crear Cotización</h1>
  ...
  <div class="row eagle-row" style="font-size: 0.8rem">
    ...
    <form class="col-12" style="padding: 0" action="{{ route('cotis.store') }}" method="POST" autocomplete="off" id="formulario">
      {{csrf_field()}}
      <section class="col-12 row" style="margin: 0; padding: 0">
        ...
        <div class="col-2 sidebar-container"> <!-- 1st column -->
          ...
          <!--div class="row eagle-row-clean col-12">
            <label for="client" class="mac-label col-4">Compañía:</label>        
            <select id="company" name="company" class="col-8 form-control mac-form-control">
                <option value=''>Seleccion Compañía</option>
                <option value=1>Rothbaro SRL</option>
                <option value=2>RB Energía SRL</option>
            </select>
            
            <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
              <strong>Debe seleccionar una compañía.</strong>
            </span>
          ...
          <div class="row eagle-row-clean col-12">
            ...
            <select id="contact" name="contact_id" class="col-8 form-control mac-form-control" data-old="{{ old('contact_id') }}"></select>
          </div>

          <div class="row eagle-row-clean col-12">
            <label for="address" class="mac-label col-4">Dirección de entrega:</label>
            <select id="address" name="address_id" class="col-8 form-control mac-form-control" data-old="{{ old('address_id') }}"></select>
          </div>
          ...
          <div class="row eagle-row-clean col-12">         
            ...
            <input type="date" id="date" name="date" class="col-7 form-control mac-form-control" data-old="{{ old('date') }}">
              <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
          ...
          <div class="row eagle-row-clean col-12">         
            <label class="mac-label col-5">Validez Oferta [días]:</label> 
            <input id="validez" name="validez" class="col-7 form-control mac-form-control" value="{{ old('validez') ? old('validez') : '15' }}"> 
              <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                <strong>Ingrese validez de la cotización en días. Debe ser un valor numérico.</strong>
              </span>   
          </div>

          <div class="row eagle-row-clean col-12">         
            <label class="mac-label col-5">Fecha de Entrega:</label>   
            <input type="date" id="delivery_date" name="delivery_date" class="col-7 form-control mac-form-control" data-old="{{ old('delivery_date') }}">
              <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
          ...
          <!--div class="row eagle-row-clean col-12">         
            <label class="mac-label col-5">Plazo de Entrega [días]*:</label> 
            <input id="plazo" name="plazo" class="col-7 form-control mac-form-control" data-old="{{ old('plazo') }}">
              <span class="col-12 text-center" style="padding: 0; font-size: 0.7rem">
                <strong>* En caso de no conocer el plazo de entrega, ingresar "A Confirmar". Ingresando 0 (cero), la entrega es inmediata.</strong>
              </span> 
              <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                <strong>Ingrese plazo de entrega.</strong>
              </span>
          </div-->      
          ...
          <div class="row eagle-row-clean col-12">
            ...
            <select id="payment_terms" name="payment_terms" class="col-7 form-control mac-form-control">
              @foreach($salesTermsCoti->get() as $index => $oneSalesTermsCoti)            
                <option value="{{ $index }}" {{ old('salesTermsCoti') == $index ? 'selected' : '' }}>
                  {{ $oneSalesTermsCoti }}
                </option>
              @endforeach
            </select>
          ...
          <div class="row eagle-row-clean col-12">
            <label class="mac-label col-4">Descripción (opcional):</label>
            <input type="text" id="terms_desc" name="terms_desc" class="col-8 form-control mac-form-control" value="{{ old('terms_desc') }}">       
          </div>

          <hr style="margin: 0.25rem">

          <div class="row eagle-row-clean col-12">
            <label class="mac-label col-4">Observaciones:</label>
            <input type="text" id="obs" name="obs" class="col-8 form-control mac-form-control" value="{{ old('obs') }}">       
          </div>
              ...
              <table class="table" style="font-size: 0.75rem">
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="eagle-td">
                      <select name="item[]" class="form-control mac-form-control item">
                        @foreach($cotiItems->get() as $index => $cotiItem)            
                          <option value="{{ $index }}" {{ old('item') == $index ? 'selected' : '' }}>{{ $cotiItem }}</option>
                        @endforeach
                      </select>
                      <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                    ...
                    <td class="eagle-td">
                      <textarea rows="3" name="desc[]" class="form-control mac-form-control desc" style="resize: vertical; min-height: 50px;"></textarea>
                      <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                    ...
                    <!--td class="eagle-td">
                      <input name="power[]" class="form-control mac-form-control-right pot">   
                    </td-->

                    <!--td class="eagle-td">
                      <select type="text" name="regime[]" class="form-control mac-form-control regime">
                        @foreach($regimes->get() as $index => $regime)            
                          <option value="{{ $index }}" {{ old('regime') == $index ? 'selected' : '' }}>{{ $regime }}</option>
                        @endforeach
                      </select>
                      <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                        <strong>Si es alquiler, ingrese régimen</strong>
                      </span> 
                    </td-->

                    <!--td class="eagle-td">
                      <select type="text" name="units[]" class="form-control mac-form-control units">
                        @foreach($units->get() as $index => $unit)            
                          <option value="{{ $index }}" {{ old('unit') == $index ? 'selected' : '' }}>{{ $unit }}</option>
                        @endforeach
                      </select>
                      <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                        <strong>Ingrese unidades</strong>
                      </span> 
                    </td-->
                    ...
                    <!--td class="eagle-td">
                      <select type="text" name="frequency[]" class="form-control mac-form-control frequency">
                        @foreach($frequencies->get() as $index => $frequency)            
                          <option value="{{ $index }}" {{ old('frequency') == $index ? 'selected' : '' }}>{{ $frequency }}</option>
                        @endforeach
                      </select>
                      <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                        <strong>Si es servicio abono, ingrese frecuencia</strong>
                      </span> 
                    </td-->

                    <td class="eagle-td">
                      <input name="list_price[]" class="form-control mac-form-control-right list_price"> 
                    </td>

                    <td class="eagle-td">
                      <input name="of_price[]" class="form-control mac-form-control-right of_price">
                      <span class="col-12 invalid-feedback d-none text-center" role="alert" style="padding: 0">
                    ...
                    <td class="eagle-td">
                      <input name="of_price_plus_IVA[]" class="of_price_plus_IVA" hidden>
                      <input class="form-control mac-form-control-right of_price_plus_IVA_form" readonly>
                    </td>
        ...
        <div class="col-12 bordered-container row no-marg" style="margin-top: 0.25rem;"><!--3rd column-->
          <div class="col-12 row no-marg">
            ...
            <div class="col-1" style="display: flex; justify-content: flex-end">
              <input class="chbx" type="checkbox" name="op1" id="op1" value=1>
            </div>
            ...
            <div class="col-1" style="display: flex; justify-content: flex-end">
              <input class="chbx" type="checkbox" name="op2" id="op2" value=1>
            </div>
            <div class="col-2" style="display: flex; justify-content: flex-start; align-items: center ;">
            ...
            <div class="col-1" style="display: flex; justify-content: flex-end">
              <input class="chbx" type="checkbox" name="op3" id="op3" value=1>
            </div>
            <div class="col-2" style="display: flex; justify-content: flex-start; align-items: center ;">
            ...
            <div class="col-1" style="display: flex; justify-content: flex-end">
              <input class="chbx" type="checkbox" name="op4" id="op4" value=1>
            </div>
            <div class="col-2" style="display: flex; justify-content: flex-start; align-items: center ;">
        ...
        <div class="mac-button-container col-2">          
          <button class="col-8 btn eagle-button" type="submit">Crear</button>
        </div>    
...
//------------------------------------------------------------------------------
    ...
    //mut_excl_1();
    //$('#op2, #op3').on('change', mut_excl_1);

//------------------------------------------------------------------------------

    //function mut_excl_2() {
      //if ($("#op9").is(":checked")) {
        //$("#op10, #op11, #op12").prop("checked",false);
      //} else if ($("#op10").is(":checked")) {
        //$("#op11, #op12").prop("checked",false);
      //} else if ($("#op11").is(":checked")) {
        //$("#op12").prop("checked",false);
      //} 
    //}  
  
    //mut_excl_2();
    //$('#op9, #op10, #op11, #op12').on('change', mut_excl_2);
...
// Validaciones --------------------------------------------------------------------------------------------
    ...
    $("#formulario").on('submit', function(event) {
     event.preventDefault();
      ...
      var alfa = $("#validez").val();
      var beta = isNaN(alfa);
      if(!$("#validez").val() || beta == true) {
      ...
      //var gamma = $("#plazo").val();
      //var delta = isNaN(gamma);
      //if(!$("#plazo").val()) {
        //$("#plazo").addClass('is-invalid');
        //$("#plazo").parent().find('.invalid-feedback').removeClass('d-none');
        //pass = false;
      //}
      ...
      //$(".regime").each(function(index, el) {
        //if (!$(this).val() && $(".item").val() == 1) {
          //$(this).addClass('is-invalid');
          //$(this).parent().find('.invalid-feedback').removeClass('d-none');
          //pass = false;
        //}
      ...
      //$(".frequency").each(function(index, el) {
        //if (!$(this).val() && $(".item").val() == 6) {
          //$(this).addClass('is-invalid');
          //$(this).parent().find('.invalid-feedback').removeClass('d-none');
          //pass = false;
        //}
